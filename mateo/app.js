import { HISTORICAL_IMPORT } from "./historical-data.js?v=1";

const STORAGE_KEY = "mateo-baby-tracker-v1";
...
function renderImportStatus() {
  ...
  const importZone = els.importNotesButton?.closest(".import-zone");
  if (!total) {
    if (importZone) importZone.hidden = true;
    return;
  }
  if (importZone) importZone.hidden = false;
...
function maybeNotifyMedicineReminders() {
  ...
  medicineList().forEach((medicine) => {
    ...
    new Notification(`${displayMedicineName(medicine.name)} pendiente`, {
      ...
      icon: "./app-icon-192.png",
      tag: `mateo-medicine-${medicine.id}-${today}`,
    });
...
function stateFromBackupPayload(payload) {
  ...
  if (payload.app && payload.app !== "mateo-baby-tracker") throw new Error("Wrong app");
...
function exportSlug() {
  return (state.settings.babyName || "mateo")
    .trim()
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
